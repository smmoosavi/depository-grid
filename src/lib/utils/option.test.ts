...
describe('option', () => {
  ...
  describe('catchNoneError', () => {
    const doubleX = (o: Option<{ x: number }>) => catchNoneError(() => o.unwrap().x * 2);
  ...
  describe('map', () => {
    ...
    it('should returns none if function returns undefined', function () {
      expect(some(5).map(() => undefined)).toBe(none());
    });
  });
  ...
  it('should be json serializable', function () {
    expect(JSON.stringify(some(5))).toBe('{"some":5}');
    expect(JSON.stringify(none())).toBe('{"none":true}');
  });
});
